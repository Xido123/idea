...
object one {
  def main(args: Array[String]): Unit = {
    ...
    properties.setProperty("zookeeper.connect", "locakhost:2181") // Zookeeper服务器IP和端口

    properties.setProperty("group.id", "test")

    val streamKafka = env.addSource(new FlinkKafkaConsumer[String]("test",new SimpleStringSchema(),properties))
